...
def load_foil(file):
    ...
    for line in f:
        ...
        if (m != None) and rheader and not(rextra) and not(rintra):
            dim = list(map(lambda t: float(t), m.groups()))
            continue
        ...
        if (m != None) and rheader and rextra and rintra:
            ...
            continue;
    ex = list(map(lambda t: float(t[0]),extra))
    ey = list(map(lambda t: float(t[1]),extra))
    ix = list(map(lambda t: float(t[0]),intra))
    iy = list(map(lambda t: float(t[1]),intra))
